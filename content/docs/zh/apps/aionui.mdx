...
description: AionUi 教程 — 免费开源桌面办公 Agent，支持 Gemini CLI、Claude Code、Codex 等多种 AI 代理。对接小恐龙 API 实现多模型工作流。
---
...
API 密钥：在小恐龙 API 控制台获取  
API 地址：如果字段要求 OpenAI Base URL，填写 `https://api.klong.lat/v1`；如果字段说明会自动补 `/v1`，填写 `https://api.klong.lat`

不要将来源不明的 API 地址或密钥接入生产环境。给客户看的截图里，建议只展示 `sk-your-api-key` 一类占位内容。

### 配置步骤

1. **在小恐龙 API 控制台复制 API key**
...
3. **添加新的提供商**
   - 点击"添加模型"
   - 选择 OpenAI Compatible / OpenAI 兼容
   ![添加 NewAPI 提供商](/assets/aionui/newapi_provider.png)

4. **配置 API 信息**
   - API 地址：通常填写 `https://api.klong.lat/v1`
   - API 密钥：粘贴从小恐龙 API 控制台复制的 API Key

5. **添加模型**
   - 下拉选择需要添加的模型
   - 模型名称应与小恐龙 API 模型列表中的名称一致
   - 选择合适的请求协议

6. **开始使用**
   - 返回聊天页面
   - 选择已配置的小恐龙 API 模型开始对话
